setwd("D:/Users/Lenovo-89/Desktop/Study materials/Data Visualization and Analysis Methods/Group assignment/Research topic/CryoLakeCH4Temp-main/Data")
dir <- "D:/Users/Lenovo-89/Desktop/Study materials/Data Visualization and Analysis Methods/Group assignment/Research topic/CryoLakeCH4Temp-main" # Change this to your own directory
#Data link: https://doi.org/10.6084/m9.figshare.29146295.v1 
install.packages(c("sf", "spatialreg", "spdep", "nlme", "lme4", "ggplot2", "ggpubr", "dplyr", "broom", "MuMIn", "respirometry", "car", "broom.mixed", "sjPlot"))
...
install.packages("jsonlite", lib = "D:/Users/Lenovo-89/Rlib")
install.packages("rlang", lib = "D:/Users/Lenovo-89/Rlib")
...
library(devtools)
library(lmerTest)
library(jsonlite, lib.loc = "D:/Users/Lenovo-89/Rlib")
library(rlang, lib.loc = "D:/Users/Lenovo-89/Rlib")
...
write.csv(tidy_results_D, paste(dir, 'Output_Test_v1', 'Tables', 'lmer_dch4_temp_lake_2504v2.csv', sep = '/'))
...
write.csv(ea_table_diff, paste(dir, 'Output_Test_v1', 'Tables', "Diff_Ea_slide_window2.csv", sep = '/'), row.names = FALSE)
...
write.csv(tidy_results_E, paste(dir, 'Output_Test_v1', 'Tables', 'lmer_ech4_temp_lake_2504v2.csv', sep = '/'))
...
write.csv(ea_table_eb, paste(dir, 'Output_Test_v1', 'Tables', "Eb_Ea_slide_window2.csv", sep = '/'), row.names = FALSE)

#### area slide window Ea plots------
ea_table_diff <- read.csv(paste(dir, 'Output_Test_v1', 'Tables', "Diff_Ea_slide_window2.csv", sep = '/'))
ea_table_eb <- read.csv(paste(dir, 'Output_Test_v1', 'Tables', "Eb_Ea_slide_window2.csv", sep = '/'))
...
ggsave(paste(dir, 'Output_Test_v1', 'Figures', 'Ea slide window2.png', sep = '/'), ggarrange(ea_table_diff_fig,  ea_table_eb_fig, ncol = 1, nrow = 2,  labels = c("a", "b")), width = 5.5, height = 10, dpi = 300)
...
write.csv(tidy_results_D_depth, paste(dir, 'Output_Test_v1', 'Tables', 'lmer_dch4_temp_lake_bydepth2.csv', sep = '/'))
...
ea_table_diff2
write.csv(ea_table_diff2, paste(dir, 'Output_Test_v1', 'Tables', "Diff_Ea_slide_window2depth.csv", sep = '/'), row.names = FALSE)
...
write.csv(tidy_results_E_depth, paste(dir, 'Output_Test_v1', 'Tables', 'lmer_ech4_temp_lake_bydepth2.csv', sep = '/'))
...
ea_table_eb2
write.csv(ea_table_eb2, paste(dir, 'Output_Test_v1', 'Tables', "Eb_Ea_slide_window2depth.csv", sep = '/'), row.names = FALSE)


#### depth slide window Ea plots------
ea_table_diff2 <- read.csv(paste(dir, 'Output_Test_v1', 'Tables', "Diff_Ea_slide_window2depth.csv", sep = '/'))
ea_table_eb2 <- read.csv(paste(dir, 'Output_Test_v1', 'Tables', "Eb_Ea_slide_window2depth.csv", sep = '/'))
...
ggsave(paste(dir, 'Output_Test_v1', 'Figures', 'Ea slide window depth2.png', sep = '/'), ggarrange(ea_table_diff_fig2,  ea_table_eb_fig2, ncol = 1, nrow = 2,  labels = c("a", "b")), width = 5.5, height = 10, dpi = 300)

ggsave(paste(dir, 'Output_Test_v1', 'Figures', 'Ea slide window area depth.png', sep = '/'), ggarrange(ea_table_diff_fig, ea_table_diff_fig2, ea_table_eb_fig, ea_table_eb_fig2, ncol = 2, nrow = 2,  labels = c("a", "b", "c", "d"), font.label = list(size = 20, color = "black")), width = 10, height = 10, dpi = 300)
...
site_Ea_tch4_L <-  Lakedat2_clean_t %>%
  drop_na(ikt) %>% 
  group_by(Site_ID) %>%
  do(tidy(lm(log(Total_CH4_mmolm2d) ~ ikt, data = .))) %>%
  filter(term == "ikt") %>%
  select(Site_ID, slope = estimate, p_value=p.value)
site_Ea_tch4_L <- as.data.frame(site_Ea_tch4_L) %>% drop_na(slope) %>% drop_na(p_value)
plot(density(site_Ea_tch4_L$slope))
...
ggsave(paste(dir, 'Output_Test_v1', 'Figures', 'lake standardized temp vs. flux10.png', sep = '/'), ggarrange(ggarrange(L_dch4_t, L_ech4_t, L_tch4_t, ncol = 1, nrow = 3, common.legend = TRUE, legend="bottom", labels = c("a", "b", "c"), heights = c(0.85,0.81,0.92)) , ggarrange(pds_dch4_2, pds_ech4_2, pds_tch4_2, ncol = 1, nrow = 3, labels = c("d", "", "")),  ncol=2, nrow=1, widths = c(1,0.7), align = "hv"), width = 6.1, height = 11.5, dpi = 300)
...
ggsave(paste(dir, 'Output_Test_v1', 'Figures', 'lake size temp vs. log10 flux5-1.png', sep = '/'), grid.arrange(arrangeGrob(plot, bottom = x.grob, top = top.grob)), device = "png", width = 9.3, height = 13.5, dpi = 300)
...
ggsave(paste(dir, 'Output_Test_v1', 'Figures', 'lake depth temp vs. log10 flux-4.png', sep = '/'), grid.arrange(arrangeGrob(plot, bottom = x.grob, top = top.grob)), device = "png", width = 5.5, height = 10, dpi = 300)
...
Lakedat2 <- read.csv("path_to_your_data.csv")  # Make sure to use the correct path to your data file

# 1. Parallel Coordinates Plot
# Ensure the data for parallel coordinates is clean
data_for_parallel <- Lakedat2[, c("CH4_uM", "CO2_uM", "N2O_uM", "Salinity_PSU", "Depth_max_m")]
data_for_parallel <- na.omit(data_for_parallel)  # Remove NA values
data_for_parallel <- as.data.frame(t(data_for_parallel))

# Create parallel coordinates plot using ggplot2
parallel_plot <- ggplot(Lakedat2, aes(x = CH4_uM, y = CO2_uM, color = factor(sizeclass10))) +
  geom_point() +
  labs(title = "Parallel Coordinates for Lake Characteristics") +
  theme_minimal()

# Save Parallel Coordinates Plot using ggsave
ggsave(paste(dir, 'Output_Test_v1', 'NEW', 'Parallel_Coordinates_Plot.png', sep = '/'), parallel_plot, device = "png", width = 5.5, height = 5.5, dpi = 300)

# 2. Facet Grid Plot
facet_grid_plot <- ggplot(Lakedat2, aes(x = CH4_uM, y = CO2_uM, color = sizeclass10)) +
  geom_point() +
  facet_grid(sizeclass10 ~ .) +
  labs(title = "Faceted Scatter Plot of Methane vs Carbon Dioxide by Lake Size Class", 
       x = "Methane Concentration (µM)", y = "CO2 Concentration (µM)") +
  theme_minimal()

# Save Facet Grid Plot using ggsave
ggsave(paste(dir, 'Output_Test_v1', 'NEW', 'Facet_Grid_Plot.png', sep = '/'), facet_grid_plot, device = "png", width = 5.5, height = 5.5, dpi = 300)

# 安装并加载所需包
install.packages(c("ggplot2", "dplyr", "ggpubr", "ggcorrplot", "leaflet", "plotly", "sf", "tidyr"))
library(ggplot2)
library(dplyr)
library(ggpubr)
library(ggcorrplot)
...
# 读取数据
Lakedat2 <- read.csv("path_to_your_data.csv")  # 请确保路径正确
Lakedat2_clean_d <- Lakedat2[complete.cases(Lakedat2[, c("Diff_CH4_mmolm2d", "ikt", "Longitude", "Latitude", "Site_ID")]), ]
Lakedat2_clean_e <- Lakedat2[complete.cases(Lakedat2[, c("Eb_CH4_mmolm2d", "ikt", "Longitude", "Latitude", "Site_ID")]), ]
...
# 3. Radar Chart (Ensure there are at least 3 variables for radar chart)
lake_features <- Lakedat2[1, c("CH4_uM", "CO2_uM", "N2O_uM", "Salinity_PSU", "Depth_max_m")]
lake_features <- as.data.frame(t(lake_features)) 
lake_features <- rbind(rep(100, 5), rep(0, 5), lake_features) 

# Create radar chart (fmsb package)
radar_plot <- radarchart(lake_features, axistype = 1, 
             pcol = "blue", pfcol = rgb(0.2, 0.5, 0.2, 0.5), plwd = 4,
             title = "Radar Chart of Lake Characteristics")

# Save Radar Chart using ggsave
ggsave(paste(dir, 'Output_Test_v1', 'NEW', 'Radar_Chart.png', sep = '/'), radar_plot, device = "png", width = 5.5, height = 5.5, dpi = 300)

# 4. Stacked Bar Plot
Lakedat2_long <- Lakedat2 %>%
  gather(key = "Gas", value = "Emission", CH4_uM, CO2_uM, N2O_uM) %>%
  na.omit()  # Remove NA values

stacked_bar_plot <- ggplot(Lakedat2_long, aes(x = sizeclass10, y = Emission, fill = Gas)) +
  geom_bar(stat = "identity") +
  labs(title = "Stacked Bar Plot of Gas Emissions by Lake Size Class", x = "Lake Size Class", y = "Gas Emission") +
  theme_minimal()

# Save Stacked Bar Plot using ggsave
ggsave(paste(dir, 'Output_Test_v1', 'NEW', 'Stacked_Bar_Plot.png', sep = '/'), stacked_bar_plot, device = "png", width = 5.5, height = 5.5, dpi = 300)

# 5. Density Plot
density_plot <- ggplot(Lakedat2, aes(x = CH4_uM, fill = sizeclass10)) +
  geom_density(alpha = 0.5) +
  labs(title = "Density Plot of Methane Concentration by Lake Size Class", x = "Methane Concentration (µM)") +
  theme_minimal()

# Save Density Plot using ggsave
ggsave(paste(dir, 'Output_Test_v1', 'NEW', 'Density_Plot.png', sep = '/'), density_plot, device = "png", width = 5.5, height = 5.5, dpi = 300)

# 6. Geo-Distribution Map (Interactive map using leaflet)
leaflet_map <- leaflet(Lakedat2_clean_d_sf) %>%
  addTiles() %>%
  addCircleMarkers(radius = 5, color = ~colorNumeric("YlOrRd", Lakedat2_clean_d$Ea)(Lakedat2_clean_d$Ea))

# Save Geo-Distribution Map as HTML (same path format as other plots)
saveWidget(leaflet_map, paste(dir, 'Output_Test_v1', 'NEW', 'Geo_Distribution_Map.html', sep = '/'))
